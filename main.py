...
def evaluate():
    ...
    img_name = 'wxx.jpg'
    valid_lr_img = get_imgs_fn(img_name, './')  # if you want to test your own image
    ...
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
    tl.layers.initialize_global_variables(sess)
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_srgan_wgan.npz', network=net_g)
